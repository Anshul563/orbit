  ...
  return (
    ...
    >
      <nav
        ...
        className={`card-nav ${
          isExpanded ? "open" : ""
        } block h-[60px] p-0 rounded-xl backdrop-blur-sm shadow-md relative overflow-hidden will-change-[height]`}
      ...
      >
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => {
            const isLink =
              !!item.href && (!item.links || item.links.length === 0);
            const CardTag = isLink ? "a" : "div";
            const cardProps = isLink ? { href: item.href } : {};

            return (
              <CardTag
                key={`${item.label}-${idx}`}
                className="nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] no-underline"
                ref={setCardRef(idx)}
